...
Before drafting prose, record:

- **Purpose:** State the single job of the section.
- **Reader entry state:** State what the intended reader already knows.
- **Reader exit state:** State what the reader must understand or accept afterward.
- **Argument responsibility:** Identify the claims this section must establish.
- **Evidence responsibility:** Identify the definitions, sources, derivations, observations, or results it must carry.
- **Boundary:** Identify claims and details that belong elsewhere.
- **Constraints:** Respect scope, length, terminology, venue, and locked-content limits.

Read [references/section-contracts.md](references/section-contracts.md) when drafting a named manuscript section or deciding whether content is misplaced.
...
defines a needed object, advances the argument, supplies evidence, establishes
a material boundary, enables the promised reproduction level, or satisfies an
explicit artifact obligation. Correct detail that does not serve the reader or
claim is still removable.

Choose the spine that the research requires. Do not force every paper into a fixed sequence such as gap, method, result. A proof, empirical discovery, resource paper, qualitative study, system paper, or synthesis can require a different progression.

Read [references/paper-types-and-evidence.md](references/paper-types-and-evidence.md) when selecting a paper-level spine or adapting the draft across contribution types.
...
- the new proposition it contributes;
- the support it provides;
- the relation to the preceding paragraph;
- the reader state after the paragraph.
...
## Draft from structure to sentences

1. Draft topic sentences from paragraph propositions.
2. Add only the definitions, reasoning, evidence, and qualifications needed to establish each proposition.
3. Use stable technical terms even when repetition is stylistically noticeable.
4. State direct measurements separately from interpretations and hypotheses.
5. Keep claim strength within the evidence boundary.
6. Use transitions that express an actual logical relation.
7. Prefer concrete subjects and verbs over abstract praise or procedural narration.
8. Compress by removing redundancy, not by removing scientific qualifiers or interpretive context.

## Run the drafting gates

Verify before handoff:

- **Contract gate:** Make the section perform its assigned job.
